...
def create_room(game, xOFFSET, yOFFSET, room):
    ...
    for row, tiles in enumerate(room.data):
            for col, tile in enumerate(tiles):
                ...
                if tile == "D":
                    door = Door(game, col + xOFFSET, row + yOFFSET, 32, 32, game.num_of_rooms)
                ...
                if tile == "1" :
                    ...
                    if first:
                        ...
                        BOTTOMRIGHT = (wall.rect.bottomleft[0] + (16 * 32), wall.rect.topleft[1] + (16 * 32)) 
                        first = False
    ...
    if game.num_of_rooms != 1:
        for i in range(num_of_ennemies):
            Slime(game, random.randint(TOPLEFT[0] + game.settings["TILESIZE"], BOTTOMRIGHT[0] - 2 *game.settings["TILESIZE"]), random.randint(TOPLEFT[1] + game.settings["TILESIZE"], BOTTOMRIGHT[1] - 2 * game.settings["TILESIZE"]), game.player, 2, 1, game.num_of_rooms)
            total_ennemies += 1
    ...
    if "up" in room.type:
        test_cube = Room_DETECT(game, room_x + test_xoffset, room_y - test_yoffset, 32, 32)
    ...
    if "down" in room.type:
        test_cube = Room_DETECT(game, room_x + test_xoffset, room_y + (3*test_yoffset) - 1, 32, 32)
        hit = pygame.sprite.spritecollide(test_cube, game.RoomMarkers, False)
        if not hit:
    ...
    if "left" in room.type:
        ...
        if not hit:
            ...
            create_room(game, room_x - 15, room_y, game.need_right_opening[num])
    if "right" in room.type:
        test_cube = Room_DETECT(game, room_x + (3*test_xoffset) - 1, room_y + test_yoffset, 32, 32)
        hit = pygame.sprite.spritecollide(test_cube, game.RoomMarkers, False)
        if not hit:
    ...
    for row, tiles in enumerate(room.data):
            for col, tile in enumerate(tiles):
                ...
                if tile == "W":
                    ...
                    Boss_slime(game, col, row, slime_0, [(250, 250), (500, 250)], 10, 2)
...
class Game:
    ...
    def load_maps(self):
        ...
        self.start_map = MAP(self, "ROOM TEMPLATES/Start.txt", ["up", "down", "left", "right"])
        ...
        self.top_right_map = MAP(self, "ROOM TEMPLATES/TR.txt", ["up", "right"])
        self.top_left_map = MAP(self, "ROOM TEMPLATES/TL.txt", ["up", "left"])
        self.bottom_right_map = MAP(self, "ROOM TEMPLATES/DR.txt", ["down", "right"])
        self.bottom_left_map = MAP(self, "ROOM TEMPLATES/DL.txt", ["down", "left"])
        self.top_bottom_map = MAP(self, "ROOM TEMPLATES/DT.txt", ["down", "up"])
        self.left_right_map = MAP(self, "ROOM TEMPLATES/LR.txt", ["left", "right"])
        self.boss_map = MAP(self, "ROOM TEMPLATES/Boss.txt", [])

        self.need_up_opening = [self.top_map, self.top_left_map, self.top_right_map, self.top_bottom_map]
        self.need_down_opening = [self.bottom_map, self.bottom_left_map, self.bottom_right_map, self.top_bottom_map]
        self.need_left_opening = [self.left_map, self.bottom_left_map, self.top_left_map, self.left_right_map]
        self.need_right_opening = [self.right_map, self.top_right_map, self.bottom_right_map, self.left_right_map]
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1 and not self.IN_MENU:
                    if self.player.current_weapon.ammo > 0:
                        Projectile(self, self.player.current_weapon.shoot_point[0], self.player.current_weapon.shoot_point[1], bullet, 60)
                ...
                if event.button == 5:
                    ...
                    if self.weapon_inventory_spot < 0:
                        self.weapon_inventory_spot = len(self.weapon_inventory) - 1
    ...
    def update(self):
        ...
        try:
            self.player.current_weapon = self.weapon_inventory[self.weapon_inventory_spot]
            self.weapon_UI.image = pygame.image.load(self.Weapons[self.weapon_inventory[self.weapon_inventory_spot].name]["Icon"])
        except:
            pass
    ...
    def draw_pause_menu(self):
        ...
        self.screen.blit(Pause_Text, (int(512 - (self.FONT.size('PAUSE')[0] / 2)), 64))
    ...
    def draw_character_selection_menu(self):
        ...
        WITCH = Button(self, 350, 350, Witch_portrait, self.choose_character, need_argument = True, argument=self.Characters["Witch"])
        GREEN = Button(self, 650, 350, Green_portrait, self.choose_character, need_argument = True, argument=self.Characters["Green Test"])
    ...
    def draw_stats(self):
        ...
        try:
            ...
            room = self.FONT.render("Room: " + str(self.current_room), True, (255,255,255))
            HP = self.FONT.render("HP: " + str(self.player.HP), True, (222,255,255))
            timer = self.FONT.render("Timer: " + str(self.player.hurt_timer), True, (255,255,255))
            fps = self.FONT.render("FPS:" + str(self.clock.get_fps()), True, (255,255,255))
            total_rooms = self.FONT.render("nb of rooms: " + str(self.num_of_rooms), True, (255,255,255))
            ennemies_in_room = self.FONT.render("Ennemies: " + str(self.floor[self.current_room]["Ennemies"]), True, (255,255,255))
            total_weapons = self.FONT.render("Total weapons: " + str(len(self.weapon_inventory)), True, (255, 255, 255))
            score = self.FONT.render("Score: " + str(self.finished_floors), True, (255, 255, 255))
    ...
    def draw(self):
        ...
        if self.IN_MENU:
            ...
            ammo = self.FONT.render(str(self.player.current_weapon.ammo) + " / " + str(self.player.current_weapon.max_ammo), True, (255,255,255))
